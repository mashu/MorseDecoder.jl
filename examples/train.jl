...
#   [--checkpoint-dir checkpoints] [--save-every 1000]
#
...
function parse_args()
    ...
    (; gpu, steps, batch_size, accum_steps, checkpoint_dir, save_every)
end
...
function main()
    ...
    println("Training for $(args.steps) steps on $(args.gpu ? "GPU" : "CPU") (batch=$(args.batch_size), accum=$(args.accum_steps), effective_batch=$effective_batch, save_every=$(args.save_every))")
    ...
    for step in step_start:args.steps
        ...
        result = Flux.withgradient(model) do m
            train_step(m, spec, decoder_input, decoder_target, station_mask) / Float32(args.accum_steps)
        end
        args.gpu && CUDA.synchronize()
