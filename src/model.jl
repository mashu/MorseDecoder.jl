...
"""Vocabulary size: chars + blank (CTC) + start + padding token."""
const VOCAB_SIZE = NUM_CHARS + 3
const START_TOKEN_IDX = NUM_CHARS + 2
const PAD_TOKEN_IDX = NUM_CHARS + 3  # padding in decoder input/target; mask in loss
...
Mean cross-entropy over valid (b,k) positions, ignoring pad_idx. Branch-free denominator.
"""
